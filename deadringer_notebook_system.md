...
**Critical difference:** PW never exceeds ~15 Court Record items per case. Dead Ringer generates roughly **60–80 notebook entries by Chapter 11**. The UI cannot treat every entry as equally prominent, equally presentable, or equally findable. That assumption is what breaks a naive port of the PW interface to this game.
...
*Paired evidence (Higashino-lens additions):* Many evidence items generated by the school records investigation (Ch 5–9) exist in discoverable pairs — one documenting a hidden act of selfishness, one documenting a hidden act of love by the same character. Example: Haruki's "disruptive honesty" disciplinary label (selfishness of the institution) paired with his recommendation letter for the same student (love, complicated by futility). These are not a new entry type; they are standard EVIDENCE entries. But their pairing property matters for player experience: discovering the second item in a pair recontextualizes the first, producing an emotional asymmetry that the Log's chronological order preserves. The Board assembles them as contributing entries to existing slots, not as their own slot.
...
| Ch 4 | 25–30 entries. Character notes populating. First threads (Sora, Silver Car) assembling. Hidden act evidence begins appearing. |
| Ch 7 | 35–45 entries. Threads visibly cross-referenced. First contradictions emerging. Paired evidence items (selfishness + love) clustering from school records investigation. |
| Ch 9 | 50–65 entries. Evidence density peaks. The Ogawa thread may or may not be full depending on player depth. Emotional asymmetry observations accumulating. |
| Ch 11 | 60–75 entries. The Board assembly begins. |
| Ch 12 | 65–80 entries. Final entries from switchboard duel. |
...
**Yui-sequence first entry (Higashino-lens addition):** The first entry Reiko encounters when she opens Mira's notebook in Ch 11 is now about Yui: *"I told Mr. Ise about her. He called her mom. Her mom called him. Now it's worse."* This is not an accident of page order — Mira's most recent personal entry is about the last child she tried to help before she disappeared. The entry recontextualizes everything Reiko reads afterward: every subsequent observation in Mira's notebook carries the weight of someone who already knows that reporting makes things worse and kept reporting anyway. The UI must present this entry first in the Ch 11 reading sequence, with enough pause for the player to absorb it before the page turns.
...
|---|---|---|---|
| 1 | **IMPOSSIBLE KNOWLEDGE** | Endo knows things with the wrong resolution | Ch 8 Tell #1, Ch 9 Tell #3, Mira's notebook cross-ref, written-vs-heard distinction. *Contributing entries (Higashino-lens):* Endo's selective truth partition pattern — Ch 8 three leads each given audience-specific framing; Ch 12 intercepts confirming he quotes back what Doi said privately, verbatim, to people who weren't present |
...
| 3 | **COMMITTEE AS MECHANISM** | Same committee, opposite outcomes for teachers vs. reports | Three dismissed child reports (Ch 5), Ogawa firing (Ch 5), Haruki's committee records (Ch 11 assembly). *Contributing entries (Higashino-lens):* Haruki's "disruptive honesty" disciplinary label + recommendation letter pair (the institution weaponized the label while ignoring the advocacy); Aizawa's altered report showing the committee used the classification to justify inaction |
...
| 6 | **FRAMING AUTHORSHIP** | Every planted document traces to council-level access | Counselor notes fabrication (Ch 9), chat log metadata (Ch 9), behavioral report formatting (Ch 9), "disruptive honesty" phrase match (Ch 5+Ch 9). *Contributing entries (Higashino-lens):* Rina's notebook evidence documenting the origin of the social credibility deficit; the Yui-sequence dates showing how the "disruptive" classification originated and propagated through institutional channels before any framing documents appeared |
| 7 | **SOCIAL ACCESS (WITNESS)** | People told Endo everything because not telling felt strange | Doi's testimony (Ch 11 assembly), Reiko's community interactions (Ch 11). *Contributing entries (Higashino-lens):* Doi's council intake form (reported directly to Endo, creating a documented information pathway); Kaito's phone records (knew everything from late-night calls with Mira, still said no when it mattered — the selfishness evidence that complicates his love evidence) |
...
Hidden act evidence items begin clustering in Ch 5–7 as the school records investigation progresses. These arrive as paired discoveries — a selfishness item and a love item for the same character in close proximity — creating an emotional rhythm where each revelation immediately complicates the last. The pairing is densest during Ch 6–7, when Kaito's phone records, Haruki's disciplinary file, and Doi's intake forms all surface within a short investigative window.
...
Endo's selective truth partition pattern becomes trackable through the notebook in this act. The Ch 8 entries about his three leads — each framed differently for a different audience — create a visible pattern when the player filters the Log by the Endo tag. The same information, repackaged three ways, logged as three separate entries. A player who notices the pattern in the Log has already begun the work the Board will formalize in Ch 11. The Ch 12 intercepts later confirm the pattern in real time.
...
**Architecture:** Two layers — Log (persistent, 60–80 entries) and Board (confrontation-only, 6–8 slots).
